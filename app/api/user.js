...
api.login = async (req, res) => {
    ...
    console.log('####################################');
    const user = await new UserDao(req.db).findByEmailAndPassword(userEmail, password);
    console.log(user);
    if(user) {
        console.log(`Email ${userEmail} authenticated`);
        ...
        console.log(`Authentication failed for email ${userEmail}`);
        console.log('No token generated');
        res.status(401).json({ message: `Authentication failed for email ${userEmail}`});  
    }
};
